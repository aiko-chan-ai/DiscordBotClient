...
const patchList = ['02be0d5b4681a76d9def', '087faa3fe576396cad3c'];
...
async function start(port, log_) {
	...
	if (!html) {
		html = await getData(
			'https://raw.githubusercontent.com/aiko-chan-ai/DiscordBotClient/main/404.html',
		);
	}
		...
		await Promise.all(
			patchList.map(async (script) => {
				scriptTarget[script] = await getData(
					`https://raw.githubusercontent.com/aiko-chan-ai/DiscordBotClient/main/script/${script}.js`,
				);
			}),
		);
	}
